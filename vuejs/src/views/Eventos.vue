        ...
        <div class="mx-auto text-center">
          <b-table striped hover :fields="fields" :items="eventos">
            ...
            <template slot="Endereço" slot-scope="data">
              <p>{{data.item.region.region_address}}</p>
              <md-button
      ...
      <modal v-if="newEvent" @close="newEventHide">
        ...
        <template slot="body">
          <md-content class="md-scrollbar">
            ...
            <b-form-group id="input-group-3" label="Doação:" label-for="input-3">
              <b-form-select id="input-3" v-model="selected_turn" :options="obterTurnos()" required></b-form-select>
            </b-form-group>
            <md-field>
              <label>Data:</label>
...
export default {
  ...
  methods: {
    ...
    obterEventos() {
      this.$http
        .get("event?iid=" + localStorage.logged_institution)
        .then(res => {
    ...
    obterTipos() {
      return [
        { value: "FOOD", text: "Alimentos" },
        { value: "CLOTHING", text: "Roupas" },
        { value: "RELIGION", text: "Religiosa" },
        { value: "OTHERS", text: "Outros Tipos" }
      ];
    },
    obterTurnos() {
      return [
      ...
      if (localStorage.logged_institution && localStorage.token) {
        this.$http
          ...
          .then(res => {
            ...
            this.$http
              .get("institution/" + this.selected_institution)
              .then(res => {
    ...
    excluir(id) {
      this.$http.delete(`/event/${id}`).then(() => {
        this.editEventHide();
        this.obterEventos();
      });
    },
